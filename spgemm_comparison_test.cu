// spgemm_comparison_test.cu - Main testing program
// Compares spgemm_hash(adj,feature_matrix,output), spgemm_hash_aia, and cuSPARSE
...
// cuSPARSE implementation for comparison
template <class idType, class valType>
PerformanceResult spgemm_cusparse(CSR<idType, valType> adj, CSR<idType, valType> features, 
    ...
    result.aia1_time_ms = 0;  // No AIA in cuSPARSE
...
// Hash implementation without AIA (use existing HashSpGEMM_volta_old.hpp UNMODIFIED)
template <class idType, class valType>
PerformanceResult spgemm_hash_old(CSR<idType, valType> adj, CSR<idType, valType> features, 
                                  CSR<idType, valType> &output, const std::string& dataset, float sparsity) {
    PerformanceResult result;
    ...
    // Execute SpGEMM using UNMODIFIED old implementation
    ave_msec = 0;
    for (i = 0; i < SpGEMM_TRI_NUM; i++) {
        if (i > 0) {
            output.release_csr();
        ...
        cudaEventRecord(event[0], 0);
        SpGEMM_Hash(adj, features, output);  // Use existing function from volta_old.hpp
        cudaEventRecord(event[1], 0);
        cudaDeviceSynchronize();
        cudaEventElapsedTime(&msec, event[0], event[1]);
    ...
    result.total_time_ms = ave_msec;
    result.aia1_time_ms = 0;  // No AIA in old implementation
    result.aia2_time_ms = 0;
    result.gflops = (float)(flop_count) / 1000 / 1000 / ave_msec;
...
void run_spgemm_benchmark(const std::string& dataset_name, const std::string& data_path) {
    ...
    for (float sparsity : sparsities) {
        ...
        CSR<IT, VT> output1, output2, output3;
        
        // 1. cuSPARSE baseline
        PerformanceResult cusparse_result = spgemm_cusparse(adj, features, output1, dataset_name, sparsity);
        print_result(cusparse_result);
        
        // 2. Hash without AIA (using UNMODIFIED volta_old.hpp)
        PerformanceResult hash_old_result = spgemm_hash_old(adj, features, output2, dataset_name, sparsity);
        print_result(hash_old_result, cusparse_result.total_time_ms);
        
        // 3. Hash with AIA (using modified volta.hpp)
        PerformanceResult hash_aia_result = spgemm_hash_aia_timed(adj, features, output3, dataset_name, sparsity);
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "SpGEMM Performance Comparison: Hash vs Hash+AIA vs cuSPARSE" << std::endl;
