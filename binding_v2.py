...
extern "C" {
    // From spmm_maxk.cu (forward pass - uses CSR metadata)
    ...
    // From spmm_maxk_backward.cu (backward pass - should use CSC metadata)
    ...
    // TopK kernel from maxk_kernel.cu
...
torch::Tensor spmm_maxk_forward(
    torch::Tensor warp4_metadata,     // CSR warp scheduling metadata
    ...
    int sparse_dim = input_data.size(1);
    int num_e = indices.size(0);
    
    // Create output tensor with FULL dimensions (256), not sparse dimensions
    const int FULL_DIM = 256;
    ...
    // Call CUDA kernel (forward uses CSR metadata)
    spmm_kernel_opt2_sparse_v3_wrapper(
        ...
        num_v, num_e, FULL_DIM, dim_sparse, num_warps,
        grid, block, shared_size
    );
    
    // Check for CUDA errors
    cudaError_t error = cudaGetLastError();
    TORCH_CHECK(error == cudaSuccess, "CUDA forward kernel failed: ", cudaGetErrorString(error));
    
    return output;
}

torch::Tensor spmm_maxk_backward(
    torch::Tensor warp4_metadata,     // CSC warp scheduling metadata (CHANGED)
    torch::Tensor indices,            // Graph indices (CSC format for transpose)
    torch::Tensor values,             // Edge values
    ...
    // Call CUDA kernel (backward uses CSC metadata)
    spmm_kernel_opt2_sparse_backward_v3_wrapper(
        warp4_metadata.data_ptr<int>(),  // CSC metadata for A^T operation
        indices.data_ptr<int>(),         // CSC indices
        values.data_ptr<float>(),
    ...
    // Check for CUDA errors
    cudaError_t error = cudaGetLastError();
    TORCH_CHECK(error == cudaSuccess, "CUDA backward kernel failed: ", cudaGetErrorString(error));
    
    return grad_input;
}

// TopK kernel wrapper functions
...
torch::Tensor load_warp4_metadata(const std::string& graph_name, 
    ...
    if (!file.is_open()) {
        throw std::runtime_error("Cannot open CSR warp4 file: " + warp4_file);
    }
...
// NEW: Load CSC metadata (for backward pass)
torch::Tensor load_warp4_metadata_csc(const std::string& graph_name, 
                                      int num_warps = 12, int warp_max_nz = 64) {
    // CSC metadata path for backward pass
    std::string meta_dir = "kernels/w" + std::to_string(num_warps) + "_nz" + 
                          std::to_string(warp_max_nz) + "_warp_4_csc/";
    std::string warp4_file = meta_dir + graph_name + ".warp4_csc";
    
    // Read binary file
    std::ifstream file(warp4_file, std::ios::binary);
    if (!file.is_open()) {
        throw std::runtime_error("Cannot open CSC warp4 file: " + warp4_file + 
                                ". Ensure generate_meta_csc.py has been run.");
    }
...
// Enhanced benchmark function with CSC support
...
// Enhanced validation function with CSC support
...
// NEW: CSC validation function for backward pass
bool validate_spmm_maxk_backward(
    torch::Tensor warp4_metadata_csc,
    torch::Tensor indices_csc,
    torch::Tensor values_csc,
    torch::Tensor grad_output,
    torch::Tensor sparse_selector,
    torch::Tensor reference_grad_input,
    int num_warps_csc,
    int dim_sparse,
    float tolerance = 0.001f
) {
    auto maxk_grad_input = spmm_maxk_backward(warp4_metadata_csc, indices_csc, values_csc, 
                                             grad_output, sparse_selector, num_warps_csc, dim_sparse);
    
    auto diff = torch::abs(maxk_grad_input - reference_grad_input);
    float max_diff = torch::max(diff).item<float>();
    float avg_diff = torch::mean(diff).item<float>();
    
    std::cout << "Backward Validation - Max diff: " << max_diff 
              << ", Avg diff: " << avg_diff << std::endl;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.doc() = "MaxK-GNN CUDA kernels with CSC metadata support for correct backward pass";
    
    // Main kernel functions
    m.def("spmm_maxk_forward", &spmm_maxk_forward, 
          "MaxK-GNN forward SPMM kernel (uses CSR metadata)",
    ...
    m.def("spmm_maxk_backward", &spmm_maxk_backward,
          "MaxK-GNN backward SPMM kernel (uses CSC metadata)", 
          pybind11::arg("warp4_metadata"), pybind11::arg("indices"), pybind11::arg("values"),
    ...
    // Metadata loading functions - ENHANCED
    m.def("load_warp4_metadata", &load_warp4_metadata,
          "Load CSR warp4 metadata from file (for forward pass)",
          pybind11::arg("graph_name"), pybind11::arg("num_warps") = 12, pybind11::arg("warp_max_nz") = 64);
    
    m.def("load_warp4_metadata_csc", &load_warp4_metadata_csc,
          "Load CSC warp4 metadata from file (for backward pass)",
          pybind11::arg("graph_name"), pybind11::arg("num_warps") = 12, pybind11::arg("warp_max_nz") = 64);
    ...
    m.def("validate_spmm_maxk", &validate_spmm_maxk,
          "Validate MaxK-GNN forward kernel against reference",
          pybind11::arg("warp4_metadata"), pybind11::arg("indices"), pybind11::arg("values"),
    ...
    m.def("validate_spmm_maxk_backward", &validate_spmm_maxk_backward,
          "Validate MaxK-GNN backward kernel against reference",
          pybind11::arg("warp4_metadata_csc"), pybind11::arg("indices_csc"), pybind11::arg("values_csc"),
          pybind11::arg("grad_output"), pybind11::arg("sparse_selector"), pybind11::arg("reference_grad_input"),
          pybind11::arg("num_warps_csc"), pybind11::arg("dim_sparse"), pybind11::arg("tolerance") = 0.001f);
